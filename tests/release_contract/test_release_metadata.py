...
class ReleaseMetadataContractTest(unittest.TestCase):
    def test_current_offline_compose_candidate_metadata_is_v015_before_tagging(self) -> None:
        manifest = (ROOT / "apps" / "edge-proxy" / "Cargo.toml").read_text(encoding="utf-8")
        self.assertIn('version = "0.1.5"', manifest)
        current_state = (ROOT / "docs" / "current-state.md").read_text(encoding="utf-8")
        self.assertIn("v0.1.5 candidate metadata", current_state)
        self.assertIn("but has no `v0.1.5` tag", current_state)

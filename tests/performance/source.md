...
| `contract/edge-perf.integration.test.mjs` | Verifies release Edge HTTP Host routing, final-response `Connection: close` for POST, trusted HTTPS/SNI, malformed/body-limit responses, and configured upstream-read 504 behavior. | Recreates only performance services after explicit PKI preparation and sends only a fixed non-secret POST fixture. |

...
| --- | --- | --- |
| `contract/compose-contract.test.mjs` | Verifies the performance and reusable Nextcloud Compose profiles' service, exposure, pinned-image, state-volume, and loopback host-port contracts. | Invokes Docker Compose configuration only with test-owned host-port values, so ignored local metadata cannot alter source contracts. |
| `contract/edge-perf.integration.test.mjs` | Verifies release Edge HTTP Host routing, a POST followed by a GET on the same HTTP/1.1 client socket, trusted HTTPS/SNI, malformed/body-limit responses, and configured upstream-read 504 behavior. | Recreates only performance services after explicit PKI preparation and sends only a fixed non-secret POST fixture. |
...
| `bin/c8-compare.mjs` | Audits two baseline artifacts, requires matching captured host identity, and applies the C8 regression policy. | Fails closed for incomplete/incomparable artifacts or RPS/latency/error regressions; reads ignored artifacts only. |

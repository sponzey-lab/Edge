...
| `bin/run.mjs` | Runs the fixed smoke/baseline/stress/soak profiles through the explicit fail-closed lifecycle. | Builds/starts only performance services; raw summary and metadata are written atomically under ignored `artifacts/performance/`. |

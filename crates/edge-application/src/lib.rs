...
mod backup;
pub use backup::*;
mod access_observability;
pub use access_observability::*;
mod config_lifecycle;
pub use config_lifecycle::{ApplyResult, ConfigLifecycle};
mod config_command_dispatch;
pub(crate) use config_command_dispatch::send_apply_plan;
mod config_revision_record;
pub use config_revision_record::checksum_snapshot;
pub(crate) use config_revision_record::revision_record_for_snapshot;
mod config_validation_error;
pub(crate) use config_validation_error::validation_errors_to_app_error;
mod config_apply_policy;
pub use config_apply_policy::*;
mod config_validation;
pub use config_validation::*;
mod config_rendering;
pub use config_rendering::*;
mod config_scalar_parser;
mod mvp_config_parser;
pub use mvp_config_parser::{parse_mvp_config, ConfigSource};
mod http_route_action;
pub use http_route_action::{select_http_route_action, HttpRouteAction};
mod failure_policy_normalization;
mod operational_upgrade;
pub use operational_upgrade::*;
mod certificate_import_state;
pub use certificate_import_state::*;
mod certificate_issuance;
pub use certificate_issuance::*;
mod certificate_status_decision;
pub use certificate_status_decision::*;
mod manual_certificate_import;
pub use manual_certificate_import::*;
mod proxy_host_config;
pub use proxy_host_config::*;
mod runtime_metrics;
pub use runtime_metrics::*;
mod observability_buffer;
pub use observability_buffer::*;
...
mod http01_challenge_state;
pub use http01_challenge_state::*;
...
mod health;
mod health_probe_observability;
mod health_runtime_coordinator;
mod health_runtime_metrics;
mod health_transition_observability;
mod metrics;
mod passive_health;
...
pub use health::*;
pub use health_probe_observability::*;
pub use health_runtime_coordinator::*;
pub use health_runtime_metrics::*;
pub use health_transition_observability::*;
pub use metrics::*;
...
#[cfg(test)]
use edge_domain::{
    ClientAuthPolicy, ConfigRevisionId, ConfigSnapshot, HealthCheckPolicy, ListenerProtocol,
    LogMode, MetricsConfig, PassiveHealthMode, PassiveHealthPolicy, RetryPolicy,
    UpstreamAdministrativeState, UpstreamTlsPolicy, ValidationError,
    DEFAULT_MAX_INFLIGHT_PAYLOAD_BYTES, DEFAULT_UPSTREAM_READ_TIMEOUT_MS,
};
#[cfg(test)]
...
use edge_ports::{
    ...
    AuditSink, CertificateStore, CoreCommandClient, Http01ChallengeProbe,
};
...
mod tests {
    use super::*;
    use edge_domain::{
        AcmeChallenge, AdminConfig, CertificateResolver, CertificateResolverId, HostMatch,
        Listener, ListenerId, PathMatch, ProxyHost, ProxyHostId, Route, RouteId, RouteMatch,
        RuntimeOptions, Service, ServiceId, Upstream, UpstreamId,
    };
    use edge_ports::{
        BootstrapConfigSeed, CertificateMaterial, CertificateMaterialValidator,
        ConfigRevisionRepository, MetricDescriptor, MetricEvent, MetricsSink, ResourceMetricKind,
        ResourceRejectionReason, RevisionRecord, StartupConfigPreflight, StructuredLogEvent,
        ValidatedCertificateMaterial,
    };
...
"#;
    ...
    #[test]
    fn external_admin_bind_without_auth_fails() {
        let mut snapshot = valid_snapshot("rev-1");
        snapshot.admin.bind = "0.0.0.0:9000".to_string();
        snapshot.admin.auth_required = false;
        ...
        assert!(report
            .errors
            .iter()
            .any(|error| error.code == ErrorCode::ConfigAdminExternalBindWithoutAuth));
    }
    ...
    #[test]
    fn config_command_dispatch_preserves_order_and_stops_at_first_rejection() {
        let plan = ApplyPlan {
            commands: vec![
                CoreCommand::ApplyConfigSnapshot {
                    snapshot: valid_snapshot("rev-1"),
                },
        ...
        let mut accepted = FakeCoreCommandClient::default();
        send_apply_plan(&mut accepted, &plan).unwrap();
        assert!(matches!(
            accepted.commands.as_slice(),
            [
                CoreCommand::ApplyConfigSnapshot { .. },
                CoreCommand::RefreshRouteTable
            ]
        ...
        let mut rejected = FakeCoreCommandClient {
            reject_after: Some(2),
            ..FakeCoreCommandClient::default()
        };
        let error = send_apply_plan(&mut rejected, &plan).unwrap_err();
        assert_eq!(error.code, ErrorCode::RuntimeCommandRejected);
        assert_eq!(rejected.commands.len(), 2);
    }

    #[test]
    fn config_revision_record_construction_preserves_snapshot_compatibility_fields() {
        let snapshot = valid_snapshot("rev-record");

        let record = revision_record_for_snapshot(snapshot.clone(), "apply");

        assert_eq!(record.revision.id, snapshot.revision_id);
        assert_eq!(record.revision.schema_version, snapshot.schema_version);
        assert_eq!(record.revision.summary, "apply rev-record");
        assert_eq!(record.checksum, checksum_snapshot(&snapshot));
        assert_eq!(record.snapshot, snapshot);
    }

    #[test]
    fn config_validation_error_conversion_preserves_first_error_and_empty_fallback() {
        let errors = vec![
            ValidationError::new(ErrorCode::ConfigInvalidUpstreamUrl, "first error"),
            ValidationError::new(ErrorCode::ConfigInvalidLogMode, "later error"),
        ];

        let first = validation_errors_to_app_error(&errors);
        assert_eq!(first.code, ErrorCode::ConfigInvalidUpstreamUrl);
        assert_eq!(first.message, "first error");

        let fallback = validation_errors_to_app_error(&[]);
        assert_eq!(fallback.code, ErrorCode::InternalBug);
        assert_eq!(fallback.message, "unknown validation error");
    }

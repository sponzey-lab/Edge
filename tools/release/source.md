...
| `.github/workflows/build-binaries.yml` release job | On a strict `vMAJOR.MINOR.PATCH` tag only, publishes the Linux multi-architecture GHCR image and assembles its binary release assets from the returned digest. | The only CI job with `packages: write`; PRs retain read-only permissions and no runtime secret is passed to the assembler. |

...
| `validate_promotion_evidence.py` | Validates that one candidate's tracked evidence covers source quality, private-PKI scope, support bundle, and each supported Linux amd64 clean-host matrix cell. | Reads one supplied JSON document and explicit immutable identities; fails closed without GitHub or product-data access. |

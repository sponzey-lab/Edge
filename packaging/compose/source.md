...
| `prepare-upgrade.sh` | Updates only fixed Compose control-plane assets from a verified target archive. | Requires root and replaces the fixed Compose files and helper; does not touch runtime image state, canonical primary config, or persistent data. |
| `docker-compose.upgrade.yml` | Fixed upgrade-only Compose override. | Runs only the short-lived backup/verify invocation as UID 0 with `DAC_OVERRIDE`, so it can read the root-owned owner-only fixed passphrase and open the edge-owned exclusive data lock; normal serving remains non-root and never uses this override. |

...
output_dir=${1:-artifacts/memory-evidence/task046-current}
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/sponzey-https-steady.XXXXXX")
proxy_pid=
upstream_pid=
driver_pid=
sampler_pid=
...
openssl req -x509 -newkey rsa:2048 -nodes -days 2 -sha256 \
  -subj '/CN=Sponzey HTTPS Steady Root' -keyout "$tmp_dir/root.key" -out "$tmp_dir/root.pem" \
...
cargo build --release -p edge-proxy -p edge-memory-harness \
  --bin edge-proxy --bin edge-https-steady --bin edge-memory-evidence
...
proxy_pid=$!
wait_port "$https_port" "$proxy_pid"
wait_port "$admin_port" "$proxy_pid"

./target/release/edge-https-steady \
  --address "127.0.0.1:$https_port" --host localhost --server-name localhost \
  --root-pem "$tmp_dir/root.pem" --requests 50000 --workers 100 \
...
while time.time()<deadline:
  try: os.kill(pid,0)
  except OSError: raise SystemExit("HTTPS steady driver exited before ready")
  if os.path.exists(path) and open(path).read().strip()=="50000 100": break
  time.sleep(.05)
else: raise SystemExit("HTTPS steady driver readiness timed out")
' "$tmp_dir/ready" "$driver_pid"

report="$output_dir/https-steady-v1.json"
digest="$output_dir/https-steady-v1.sha256"
./target/release/edge-memory-evidence sample --pid "$proxy_pid" --scenario https-steady \
...
while time.time()<deadline:
  ...
  time.sleep(.05)
else: raise SystemExit("HTTPS steady status monitor timed out")
...
./target/release/edge-memory-evidence validate --scenario https-steady --scenario-version phase011-v1 \
  --build-identity "$build_identity" --config-sha256 "$config_sha256" --report "$report" --digest "$digest"
cp "$tmp_dir/driver-summary.json" "$output_dir/https-steady-driver-summary.json"
...
' "$upstream_port")
test "$trusted_forwarded" -eq 50000

rejected_negatives=0
if curl -fsS --max-time 5 --resolve "localhost:$https_port:127.0.0.1" \
  "https://localhost:$https_port/" -o /dev/null 2>/dev/null; then exit 1; else rejected_negatives=$((rejected_negatives + 1)); fi
if curl -fsS --max-time 5 --cacert "$tmp_dir/root.pem" \
  "https://127.0.0.1:$https_port/" -o /dev/null 2>/dev/null; then exit 1; else rejected_negatives=$((rejected_negatives + 1)); fi
test "$rejected_negatives" -eq 2
after_negatives=$(python3 -c '
import http.client,sys
c=http.client.HTTPConnection("127.0.0.1",int(sys.argv[1]),timeout=5); c.request("GET","/__count",headers={"Connection":"close"}); r=c.getresponse(); print(r.read().decode()); c.close(); assert r.status==200
' "$upstream_port")
test "$after_negatives" -eq 50000

curl -fsS --max-time 10 --cacert "$tmp_dir/root.pem" \
...
assert (d["expected"],d["succeeded"],d["failed"],d["workers"],d["state"])==(50000,50000,0,100,"completed")
assert r["peak_rss_bytes"]<=402653184
assert int(s[1])>0 and int(s[2])<=134217728 and s[5]=="normal"
' "$output_dir/https-steady-driver-summary.json" "$report" "$tmp_dir/status.txt"

peak=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["peak_rss_bytes"])' "$report")
set -- $(cat "$tmp_dir/status.txt")
summary="$output_dir/https-steady-summary.txt"
printf '%s\n' "HTTPS steady passed expected=50000 succeeded=50000 failed=0 workers=100 rejected_negatives=2 forwarded=50000 samples=$1 max_active=$2 max_charge=$3 final=0/0/normal recovery=200 peak_rss_bytes=$peak" > "$summary"
if rg -ni 'authorization|cookie|private_key|root.pem|config_file|passphrase|secret|"pid"|/tmp/|BEGIN CERTIFICATE|BEGIN PRIVATE' \
  "$report" "$digest" "$output_dir/https-steady-driver-summary.json" "$summary"; then exit 1; fi
cat "$summary"
printf '%s\n' "Phase 011 HTTPS steady memory smoke passed"

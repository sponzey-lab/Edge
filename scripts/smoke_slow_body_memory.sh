...
output_dir=${1:-artifacts/memory-evidence/task036-current}
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/sponzey-slow-body.XXXXXX")
proxy_pid=
upstream_pid=
driver_pid=
...
cargo build --release -p edge-proxy -p edge-memory-harness \
  --bin edge-proxy --bin edge-slow-body --bin edge-slow-body-cycles --bin edge-memory-evidence
...
while [ "$cycle" -le 5 ]; do
  ...
  driver_log="$tmp_dir/cycle-$cycle-driver.log"
  held_status="$tmp_dir/cycle-$cycle-held.status"
  final_status="$tmp_dir/cycle-$cycle-final.status"
  ./target/release/edge-slow-body \
    --address "127.0.0.1:$proxy_port" \
    --connections 128 \
    --declared-body-bytes 65536 \
    --sent-body-bytes 32768 \
  ...
  attempts=0
  while [ ! -f "$ready" ]; do
    kill -0 "$driver_pid" 2>/dev/null || { printf '%s\n' "slow body driver exited before ready" >&2; exit 1; }
    attempts=$((attempts + 1))
    test "$attempts" -le 600 || { printf '%s\n' "slow body driver ready timeout" >&2; exit 1; }
    sleep 0.1
  done
  test "$(tr -d '\n' < "$ready")" = "128 32768"
  poll_status "$admin_port" 128 1 "$held_status"

  held_report="$output_dir/slow-body-cycle-$cycle-held-v2.json"
  held_digest="$output_dir/slow-body-cycle-$cycle-held-v2.sha256"
  ./target/release/edge-memory-evidence sample \
    --pid "$proxy_pid" --scenario "slow-body-cycle-$cycle-held-128" --scenario-version phase011-v1 \
  ...
  ./target/release/edge-memory-evidence validate \
    --scenario "slow-body-cycle-$cycle-held-128" --scenario-version phase011-v1 \
    --build-identity "$build_identity" --config-sha256 "$config_sha256" \
    --report "$held_report" --digest "$held_digest"

  wait "$driver_pid"
  driver_pid=
  rg -qF 'slow body completed expected=128 succeeded=128 failed=0' "$driver_log"
...
' "$proxy_port"
  ...
  cooldown_report="$output_dir/slow-body-cycle-$cycle-cooldown-v2.json"
  cooldown_digest="$output_dir/slow-body-cycle-$cycle-cooldown-v2.sha256"
  ./target/release/edge-memory-evidence sample \
    --pid "$proxy_pid" --scenario "slow-body-cycle-$cycle-cooldown" --scenario-version phase011-v1 \
    --build-identity "$build_identity" --config-sha256 "$config_sha256" \
    --samples 3 --interval-ms 100 --output "$cooldown_report" --digest-output "$cooldown_digest"
  ./target/release/edge-memory-evidence validate \
    --scenario "slow-body-cycle-$cycle-cooldown" --scenario-version phase011-v1 \
    --build-identity "$build_identity" --config-sha256 "$config_sha256" \
...
for cycle in range(1,6):
    held=json.load(open(os.path.join(output_dir,f"slow-body-cycle-{cycle}-held-v2.json")))
    cooldown=json.load(open(os.path.join(output_dir,f"slow-body-cycle-{cycle}-cooldown-v2.json")))
    ...
    assert status[1] == "128" and int(status[2]) >= 128*32768 and final[1:] == ["0","0"]
    observations.append({"cycle_index":cycle,"build_identity":build,"config_sha256":config,
      ...
      "succeeded":128,"failed":0,"held_payload_bytes":int(status[2]),
...
cycle_report="$output_dir/slow-body-5cycle-v1.json"
cycle_digest="$output_dir/slow-body-5cycle-v1.sha256"
./target/release/edge-slow-body-cycles collect --input "$tmp_dir/cycles.json" \
  --output "$cycle_report" --digest-output "$cycle_digest"
./target/release/edge-slow-body-cycles validate --build-identity "$build_identity" \
...
printf '%s\n' "Phase 011 slow body five-cycle memory smoke passed"

...
| `test_cargo.sh` | Supplies pinned Rust Docker builds with persistent test-only dependency caches and a grep-compatible `rg` fallback for minimal Linux hosts. | Test adapters only; builds source-controlled harness binaries in the current mounted worktree, preserves fail-closed evidence scans, and never changes product runtime configuration. |
...
| `smoke_slow_header_memory.sh` | Performs one warm-up and five fixed 256-connection slow-header cycles, then publishes the canonical plateau report/digest. | Starts only isolated local test processes; fails closed on hold, cleanup, recovery, identity, or plateau failure. |
| `smoke_slow_body_memory.sh` | Performs five fixed 128-connection partial-body cycles and publishes the canonical plateau report/digest. | Starts only isolated local test processes; fails closed on 32KiB payload, cleanup, recovery, identity, or plateau failure. |
| `smoke_slow_response_memory.sh` | Performs five fixed 128-connection slow-response cycles and publishes the canonical plateau report/digest. | Runs an isolated bounded-input 4MiB response fixture; fails closed on hold, cleanup, recovery, identity, or plateau failure. |
| `smoke_connection_churn_memory.sh` | Performs five exact 10,000-request connection-churn cycles and publishes the canonical report/digest. | Starts only isolated local test processes; rejects stale identity, tampered report, cleanup, or plateau failure. |
| `smoke_private_https_idle_capacity.sh` | Holds exactly 512 private-PKI TLS connections and publishes the full-profile HTTPS-idle report/digest. | Creates ephemeral private PKI/test processes only; rejects cleanup, recovery, secret exposure, or RSS ceiling failure. |
| `smoke_websocket_memory.sh` | Runs five exact 128-tunnel WebSocket upgrade/echo cycles and publishes the full-profile WebSocket report/digest. | Starts only isolated loopback proxy and bounded WebSocket fixture processes; rejects hold, cleanup, recovery, identity, plateau, or secret-exposure failure. |
| `smoke_control_max_memory.sh` | Builds and holds the exact control-plane audit/metrics maximum fixture and publishes the full-profile report/digest. | Uses a test-tool-only temporary ledger fixture; rejects mismatched audit/metric limits, summary, evidence identity, RSS ceiling, or secret exposure. |

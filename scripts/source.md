...
| `test_cargo.sh` | Supplies a pinned Rust Docker fallback when a test host has no installed Cargo. | Test adapters only; builds source-controlled harness binaries in the current mounted worktree and never changes product runtime configuration. |
